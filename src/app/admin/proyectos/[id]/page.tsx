...
export default function EditarProyectoPage() {
  ...
  useEffect(() => {
    async function fetchProject() {
      ...
      setTowers(p.towers?.toString() ?? "");
      setFloorsPerTower(p.floors_per_tower?.toString() ?? "");
      setStatus(p.status);
      setHeroImageUrl(p.hero_image_url);
      setHeroVideoUrl(p.hero_video_url ?? "");
      setLogoUrl(p.logo_url);
      setStartingPrice(p.starting_price?.toString() ?? "");
      setStartingPriceDisplay(p.starting_price_display ?? "");
      setEnganchePercent(p.enganche_percent?.toString() ?? "");
      setReservaAmount(p.reserva_amount?.toString() ?? "");
      setReservaDisplay(p.reserva_display ?? "");
      setCuotasEnganche(p.cuotas_enganche?.toString() ?? "");
      setMaxPlazoYears(p.max_plazo_years?.toString() ?? "");
      setBankRateDisplay(p.bank_rate_display ?? "");
